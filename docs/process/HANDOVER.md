...
- Native emulator Win32 window embedding is the default display path
- Framebuffer capture abstraction remains available for gRPC/ADB fallback mode
...
├── host/graphics/     # FramebufferCapture, GrpcFramebufferCapture, AdbFramebufferCapture, VncFramebufferCapture, AngleBackend, EglLoader
...
| `src/host/ui/NativeEmulatorView.cpp` | Primary display path; embeds Android Emulator Win32 window into the QML shell |
| `src/host/graphics/GrpcFramebufferCapture.cpp` | Fallback/debug gRPC RGB888 frame stream (960px width, `--stream-capture`) |
| `src/host/graphics/AdbFramebufferCapture.cpp` | Compatibility-only raw ADB screencap fallback (1s interval) |
...
|------------|-----|----------------|
| Screenshot streaming below 60 FPS | Android Emulator screenshot API overhead | Default path is now native window embedding; stream capture is fallback-only |
...
*This document is a snapshot. For latest status, check `docs/project/STATUS.md` and `CLAUDE.md`.*
