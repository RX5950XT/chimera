...
**日期**：2026-07-11（Session 112c）
**Build**：Release PASS（MSVC + Qt 6.8.3）
**Tests**：`ctest -LE integration` **24/24 PASS**；3 integration tests 需 emulator 運行中
**定位**：即使近期修掉多個嚴重穩定性 bug，目前仍不是可投入日常使用的產品級模擬器——所有驗證都只在單一開發機、固定測試腳本下做過，未經真實日常使用長期考驗。主要是 AI-assisted Android emulator experiment / gfxstream 與 host-shell 整合整活專案。
**維護**：S112 根治「有畫面但點不動」——真根因＝gfxstream `m_refCountPipeEnabled` 硬編 false 的 RefCountPipe 失衡（guest 靜止 ≥10s 後 display CB 被 delayed-close sweep 銷毀→producer 永久停更）。已 force true（kill switch `CHIMERA_GFXSTREAM_NO_REFCOUNT_PIPE=1`）；同 build A/B 定案（fix 3/3 GREEN、killswitch 2/2 RED）。S112c 接續抓到並修掉兩個 Quick Boot 日常路徑 bug：① 跨 display flavor（stock↔`-Fast`）載入舊 snapshot 會讓 guest 整台 brick（`chimera_display_flavor.txt` marker 自動 invalidate）；② `-Fast` custom runtime 下 `adbPathForConfig` 推不出 adb 路徑，導致按 X 關窗優雅存檔靜默失敗（加 SDK fallback）。**驗證升級到真實使用者動作**：真實 Windows 滑鼠訊息點擊（含 35s 放置後）、WM_CLOSE 關窗存檔 E2E、30 分鐘混合使用 soak（6 輪操作+放置全過）皆 pass。啟動鈴聲（guest 充電/解鎖音）已於首次開機設定關閉。ES3（GLESDynamicVersion）A/B 證實零收益，維持關閉；一般 UI scroll 限制已定性為 priority 取捨（below_normal 音訊優先）非 SwiftShader 牆。
...
- **啟動時間**：S112 起 Quick Boot 為一鍵預設（AVD default_boot；首次冷開 ~34s＋關閉時自動存檔，之後 **~7.5–8.6s**；`-NoQuick` 可回全冷開）。boot 期間有 placeholder 不裸黑。S112c 修掉兩個會讓此路徑退化的 bug：跨 display flavor（stock↔`-Fast`）載入舊 snapshot 會 brick guest（marker 自動 invalidate）；`-Fast` 下按 X 關窗曾因 adb 路徑推導失敗而永遠不存檔（已加 fallback，E2E 驗證 `pass-close-saves-quickboot`）。
- **穩定性驗證**：30 分鐘混合使用 soak（6 輪「操作＋2min 放置→驗恢復」）全過、真實滑鼠訊息點擊（含 35s 放置後）gate 全過，見 `docs/references/competitor-emulator-smoothness.md` §9。
