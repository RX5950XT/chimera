...
| SDK gfxstream proxy probe | `scripts\build-chimera-gfxstream-proxy-runtime.ps1` 可建立 `build\chimera-gfxstream-proxy-runtime`，保存 stock backend 為 `libgfxstream_backend_stock.dll` 並以 proxy DLL 維持 stock exports。它可用來找 SDK 36.5.11 hook 點；typed C probes 只可觀察自然呼叫的 `stream_renderer_*` resource lifecycle / `gfxstream_backend_*` 訊號，不可主動 map/read/export handle。`initLibrary` 現在由 `gfxstream_proxy_renderlib.cpp` 的 exact C++ signature `extern "C" __declspec(dllexport) gfxstream::RenderLibPtr initLibrary()` 提供，不再有 C shim；不可改回 `void*(void*)` C 假簽名（ABI 不相容）。`CHIMERA_GFXSTREAM_PROXY_WRAP_RENDERLIB=1` / `CHIMERA_GFXSTREAM_PROXY_WRAP_RENDERER=1` 只能做本機 opt-in probe；`sharedTextureProducer=false`，不可當 1080p/60 完成證據 |
