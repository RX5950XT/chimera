...
- True Android 1080p/60 verifier：`powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\verify-true-1080p60.ps1`。預設 `-RuntimeKind Gfxstream`，必須使用 custom shared texture runtime；stock gfxstream runtime 正確結果是 fail，不可當達標。classic EmuGL 只能用 `-RuntimeKind EmuGL` 明確驗證，不能跑 modern Android 34 Play image 時不可當完成證據。**Session 85 起此 verifier 會 PASS**：它 build/install `tools\chimera-gl60-smoke`（GLES 連續渲染 app）→ `-WarmupSeconds` 排除 app 冷啟 transient → 量 steady gate `min ≥ MinEffectiveFps(57)`（容許 windowed jitter）+ `avg ≥ MinAvgEffectiveFps(59)` + `dup ≤ 5%`。Session 88 修 normal UI 後仍 PASS（min 58.8 / avg 59.6 / dup 0，`postFrameDirectGpu`，CPU readback fallback 0）。注意：60 FPS 需 guest 連續渲染，push-based idle/boot 低 FPS 屬正常，不是 verifier 退步。
...
| True 1080p/60 verifier 失敗 | 先看 `tmp\chimera-true-1080p60.log`。預設合格必須有 `Chimera gfxstream shared texture runtime ready`（或明確 `-RuntimeKind EmuGL` 時有 `Chimera EmuGL shared texture runtime ready`）、`Shared D3D11 texture display capture started`、Android `wm size >= 1920x1080`。FPS gate 是 warmup 之後的 steady window：`CHIMERA_PERF effective` 的 `min >= 57`（容許 windowed-counter jitter）、`avg >= 59`、`dup <= 5%`（見 `Get-PerfSamples` / `Assert-True1080p60Log`）。warmup boundary 由 `Exercise-DynamicGuest` 啟動 `chimera-gl60-smoke` 連續渲染後依 perf-sample 數標記（`warmup_samples_skipped=N`）；不要把 app 冷啟的首幀 stall 算進 gate。任何 raw gRPC/ADB/screenrecord fallback 都是不合格。若 log 有 `Required shared texture capture ...`，代表 strict mode 正確 fail closed，不可改成 fallback 通過。常見假失敗：guest 不是連續渲染（idle/push-based）→ effective 偏低屬正常，需用 gl60 連續渲染量 |
...
| Emulator shared texture producer 接線 | **Session 85 已接入並 PASS**：modern gfxstream `postImpl()` headless 分支在 `bridge.isEnabled() && bridge.isDirectVkReady()` 時走 `postFrameDirectGpu`（`colorBuffer->borrowForDisplay(kVk)` → `recordCopy` GPU blit → HOST_COHERENT staging → D3D11 `UpdateSubresource`），完全不走 `m_onPost` / `readback()`；kVk borrow 失敗（GLES-backed 無法借成 VK image）才退 `chimeraPublishFrameToD3D11Texture`。legacy `libOpenglRender` 的 `ChimeraSharedTextureBridge` hook 仍是 EmuGL 路徑（HAXM/WHPX 缺席而擱置）。`GpuFrameBridge` CPU pixels callback 太晚 |
...
| 想做 zero-copy（免 staging copy） | 現況 `postFrameDirectGpu` 仍有一次 `UpdateSubresource` CPU staging copy（bridge 註解：D3D11 sole owner, no Vulkan import），但已達 1080p/60（avgMs 16.2ms 在 16.67ms budget 內），staging copy **不是** FPS 瓶頸（guest 60Hz cadence 才是）。zero-copy（D3D11 shared handle 經 `VkImportMemoryWin32HandleInfoKHR` 匯入成 VkImage + keyed mutex，直接 `vkCmdBlitImage`）屬 CPU/頻寬優化，對更高解析度/多開有益，但不會提高已達標的 60 FPS，且有回歸風險與 fragile DLL rebuild；屬刻意延後的下一階段優化，不要為了「更乾淨」而動到已通過的 path |
