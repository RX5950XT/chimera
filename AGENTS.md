...
- One-click/custom runtime smoke:
  `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\start-chimera.ps1 -Fast -InteractiveFirst -SelfTest`。根目錄 `start-chimera.cmd` 預設就是這條最快可用路徑（custom gfxstream + GuestVulkan/skiavk + normal priority；`-Stock` 才回慢速 stock gRPC）。合格條件：boot completed、`wm size=1920x1080`、screenshot 非黑、Settings foreground `interactivity=ok`、0 residual process；`-Fast` 會設定 `CHIMERA_GFXSTREAM_HEADLESS_SWIFTSHADER_ES=1` 與 `CHIMERA_GUEST_VULKAN=1`，host boot 後會套 `debug.renderengine.backend=skiavk` / `debug.hwui.renderer=skiavk` 並 framework restart 一次。
...
- **日常互動可用性 verifier**（非 GL60 synthetic）：`powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\verify-interactive-ui.ps1 -Mode Fast -GuestVulkan -MeasureSeconds 25`。verifier 會設 `CHIMERA_GUEST_VULKAN=1` + `CHIMERA_GUEST_VULKAN_HOST_SETUP=0`，自己管理 skiavk restart/re-gate，避免與 host setup race；log 必須看到 `Feature 'Vulkan' (21) is overridden to 'enabled'` 才是真 GuestVulkan 對照。量測 Home→Settings→sustained scroll→app switch，輸出 `CHIMERA_DISPLAY`/`CHIMERA_INT`/`CHIMERA_INT_PRIO`；detached baseline：`guestFps=48.6 / effFps=40.3 / dup=0 / bottleneck=render`。所有 runtime verifier/self-test 必須走 `ChimeraVerifyCommon.ps1` 的 `Resolve-EmulatorConsolePort`（0=自挑 free console/ADB pair；非 0 必須 even）與 cmdline-filtered `Stop-ChimeraProcesses`；不可硬寫 `emulator-5554` 或全域 kill `emulator/qemu-system*`。`adb input swipe` 只代表測試注入路徑，不能外推成實際滑鼠手感；一次性 host mouse-drag probe 曾量到 `guestMax=116.7 / render=57.4 / dup=0`，但該測法會搶使用者滑鼠，**禁止再用**，後續若要量 host input 必須新增不移動實體游標的 internal synthetic touch hook。
