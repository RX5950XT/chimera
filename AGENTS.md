...
- One-click/custom runtime smoke:
  `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\start-chimera.ps1 -Fast -SelfTest`。根目錄 `start-chimera.cmd` 預設 = `-Fast`（custom gfxstream + guest Vulkan feature；priority 預設 below_normal 護 host audio——S108 定案，`-InteractiveFirst` 才是 normal；`-Stock` 才回慢速 stock gRPC）。合格條件：boot completed、`wm size=1920x1080`、home screenshot **內容 gate**（bytes≥20000、nonblack≥10%、luma spread≥40，防 blank/black 假 PASS）、Settings foreground `interactivity=ok`、0 residual process；`-Fast` 會設定 `CHIMERA_GFXSTREAM_HEADLESS_SWIFTSHADER_ES=1` 與 `CHIMERA_GUEST_VULKAN=1`（只加 `-feature Vulkan`）。**禁止任何 skiavk UI 切換**（runtime setprop+restart 或 `-systemui-renderer skiavk`）：playstore user image 無 root、init 不翻譯 renderengine prop，半套用（HWUI Vulkan + SF SkiaGL）會讓所有 app 視窗全黑（2026-07-02 實證，即 `-Fast` 黑屏 bug 根因）。
...
- True Android 1080p/60 verifier：`powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\verify-true-1080p60.ps1 -WarmupSeconds 15 -MeasureSeconds 120`。預設 `-RuntimeKind Gfxstream`，必須使用 custom shared texture runtime；stock gfxstream runtime 正確結果是 fail，不可當達標。合格條件：先用 ADB screencap 證明目標 app foreground 且 1920×1080 非黑可見，再量 post-warmup steady gate `min ≥ MinEffectiveFps(57)` + `avg ≥ MinAvgEffectiveFps(59)` + `dup ≤ 5%`；post-warmup `effective<=0` 直接 fail，不能過濾。**Session 101 更正：歷史「Session 89 嚴格 120 秒 PASS 59.9/60.0」量的是零幀 blit 節奏（shared texture 發佈全零、host 視窗黑），不可引用；GLES 內容修復後付每幀 GL readback 同步成本，嚴格 60 gate 目前不通過。任何 FPS 數字必須配 host 視窗像素證據（`Get-HostWindowPixelStats`）。** 注意：60 FPS 需 guest 連續渲染，push-based idle/boot 低 FPS 屬正常。
...
| NVIDIA Vulkan 已成功建立但 headless 仍低 FPS | 先分清楚 loader 與 composition gate。若 log 顯示 `glInteropSupported: true`，但同時 `useVulkanComposition: false`、`useVulkanNativeSwapchain: false`、`useVkComp=0`，瓶頸已不是 host Vulkan/GL interop 缺失，而是 renderer flags / feature gate 沒把 runtime 帶進 Vulkan composition；不要再回頭重跑 Optimus、bundled/system loader、DriverStore PATH 假說 |
| Vulkan ICD / loader 驗證又回到 `got 4 instance exts` + `vkCreateInstance res=-9` | 先檢查測試 harness 是否用 `-gpu swiftshader_indirect` 啟動。`tmp/gfxstream-src/host/gl/gl-host-common/opengl/emugl_config.cpp:398-404` 會把 `ANDROID_EMU_VK_ICD` 強制改成 `swiftshader`；要驗證 host/NVIDIA Vulkan 路徑時，不能讓 harness 先被這層污染。Session 80 已證實把 v2/v3/v4 改成 `-gpu host` 後，三者都翻成 `got 20 instance exts`、`vkCreateInstance res=0`、`vkCreateDevice res=0` |
...
| Quick Boot snapshot 壞掉、ADB offline 或啟動後空畫面 | **S112 起 Quick Boot 是一鍵預設**（`start-chimera.ps1` 自動設 `CHIMERA_QUICK_BOOT=1`，`-NoQuick`/SelfTest 除外）：走 AVD 內建 `default_boot`（零 `-snapshot*` 旗標，emulator 自管存檔）；`stop()` 走優雅 `adb emu kill`（等 ≤20s 讓存檔完成，不可改回 `TerminateProcess` 或 named `-snapshot`——後者 load 會回捲 qcow2 磁碟＝資料遺失）。**S112c 兩個關聯 bug 已修**：① 跨 display flavor（stock↔`-Fast`）載入舊 snapshot 會讓 guest 整台 brick（adb shell 永久 hang）→`VirtualMachine::start()` 用 `chimera_display_flavor.txt` marker 自動 invalidate；② `adbPathForConfig` 對 custom runtime（`-Fast`）推不出 adb 路徑會讓 graceful kill 靜默失敗、按 X 關窗永遠不存檔→已加 `sdkRootForConfig()` fallback。用 `scripts\verify-quick-boot.ps1` 驗證（must 走出貨 flavor，不可只測 stock） |
...
| 畫面有但點擊沒反應 | 普通點擊走 emulator gRPC `sendTouch`（顯示走獨立 shared texture＝**畫面在不代表輸入通，反之亦然**）。歷任根因（S106 埠被搶、S107 -Fast 顯示凍結）**皆已否證**（S108），原始症狀 0 次受控重現。**S108 起結構性自癒**：gRPC 3-strike transport breaker（每 POST 2s timeout＝hang 型死亡也可數）→ QMP/ADB tap fallback → `getStatus` 探測自動復原（劫持 E2E `fallback-pass` 實證）。**Console (5554) `event` pointer 指令是幻象通道**——回 OK 但 guest getevent 零 kernel 事件（S108 裸機實證），已從所有 pointer 鏈移除，勿再加回。拆帳法：① `CHIMERA_GRPC_INPUT_DIAG=1` 看每個 POST 的 netErr/http/grpc-status；② 輸入端到端＝PostMessage WM_LBUTTONDOWN + guest `mCurrentFocus` 行為 oracle；③ 顯示 liveness＝三指標隨時間對照（emulator stderr `kVk GPU path (frame N)` vs host `CHIMERA_PERF total` vs 螢幕 BitBlt hash）——單張 byte-identical 截圖是 idle-static 陷阱 |
...
| stock gRPC capture 負載/非預設 port 下整輪 0 幀（`total=0`） | **根因（已證）**:`main.cpp` 建 `GrpcFramebufferCapture`/`GrpcMmapFramebufferCapture` 時不可硬寫 `8554`，必須用 `g_runtimeCfg.grpcPort`（`8554 + console offset`）。否則 verifier auto-pick console 5560 → emulator gRPC 8560，但 capture 連 8554 → 幀永遠到不了（ADB 仍正常，所以可見 gate 會誤過）。次要硬化:per-request `setTransferTimeout` 與 stall watchdog 解耦（`CHIMERA_GRPC_REQUEST_TIMEOUT_MS` 預設 6000，讓負載下慢 readback 能完成）、外層 retry timer 盲目 `stop()/start()` 前查 `hasInFlight()`。皆只是 robustness，stock 仍低 FPS，不可當 1080p/60 證據 |
| True 1080p/60 verifier 失敗 | 先看 `tmp\chimera-true-1080p60.log`。預設合格必須有 `Shared D3D11 texture display capture started`、Android `wm size >= 1920x1080`、ADB screencap 非黑可見、`GPU-direct D3D11 import OK` / `path=GPU-direct`，且 raw gRPC/ADB/screenrecord/GL readback fallback 都是 0。FPS gate 是 warmup 之後的 steady window：`effective min >= 57`、`avg >= 59`、`dup <= 5%`，post-warmup `effective<=0` 必須 fail。verifier 會自動挑 free console/ADB port pair；若固定 port 看到 emulator log boot completed 但 ADB `no emulators found`，先查 `console+1` 是否被本機服務占用。若 `Failed to find ColorBuffer` 暴增成萬行，會造成 stderr log storm 掉幀；應保留 throttled diagnostic，不可每幀輸出。常見假失敗：guest 不是連續渲染（idle/push-based）→ effective 偏低屬正常，需用 gl60 連續渲染量 |
...
| Emulator shared texture producer 接線 | **Session 101 更正**：headless 分支 `postFrameDirectGpu` 前必須 `colorBuffer->flushFromGl(); colorBuffer->invalidateForVk();`——HWC compose（CompositorGl）不標 `mGlTexDirty`，缺這兩行時 `borrowForDisplay(kVk)` 借到的 VK sibling image **從未被寫入**、blit 發佈全零（S85–S99 的「可見 PASS」全是零幀節奏，gate 只驗 guest ADB 截圖+counters）。D3D11 NT handle 匯入必須 `VK_EXTERNAL_MEMORY_HANDLE_TYPE_D3D11_TEXTURE_BIT`+`VkMemoryDedicatedAllocateInfo`（`OPAQUE_WIN32` 回 VK_SUCCESS 但無 aliasing）；consumer（GuestDisplay）必須 keyed-mutex `AcquireSync(0)==S_OK`→`CopyResource` 私有副本→`ReleaseSync(0)`（不 acquire 跨 process 讀=零；`WAIT_TIMEOUT` 過得了 `SUCCEEDED()`）。診斷：bridge `debugReadbackSharedImage`（VK 端讀回）+ `invalidateForVk` exit log + texprobe/vkinterop probe 分層取證 |
| Shared texture 只有 30fps 左右 | 先確認 producer 真的以 60Hz 產生新 even sequence；consumer 端 `SharedD3D11TextureCapture` 應由 worker 等 frame event，不可退回 UI thread QTimer。Session 85：gfxstream direct-Vulkan path 在連續渲染 guest 下穩 60（avgMs ~16.2ms）；若仍只有 30，多半是 guest 沒在連續渲染或 consumer 退回 QTimer |
| GPU-direct D3D11 shared texture path | `postFrameDirectGpu`＝GL→VK 同步（GLES 內容）→ `recordCopy` GPU blit → keyed-mutex D3D11 shared texture → consumer AcquireSync+私有副本。**Session 89 的「120 秒可見 PASS min 59.9/avg 60.0」已被 Session 101 更正為零幀 blit 節奏**（host 視窗當時是黑的）。修復後互動 UI ~43 eff FPS；GLES 連續渲染受每幀 GL readback 同步限制不到 60，真 60 需 guest Vulkan-backed 內容（zero-copy）。任何「可見」宣稱必須含 `Get-HostWindowPixelStats` host 視窗像素證據 |
