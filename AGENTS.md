...
- emulator priority / 互動資源 env：`CHIMERA_INTERACTIVE_PRIORITY`（`idle|below_normal|normal`，雙擊 `start-chimera.cmd` 走 `normal`；script 無旗標預設仍是 built-in balance）、`CHIMERA_INTERACTIVE_CPUS`/`CHIMERA_INTERACTIVE_RAM_MB`（預設 4/4096，但 normalizer floor `>=4/4096`）。`CHIMERA_DISPLAY` log 一行印實際 path/priority/cpus/ram。
...
| 畫面有但點擊沒反應 | 普通點擊走 emulator gRPC `sendTouch`（fire-and-forget POST 到 `127.0.0.1:8554`），顯示走獨立的 shared texture＝**畫面在不代表輸入通**。先 headless 直開同一 /data 用 `input swipe`+screencap md5+`dumpsys window mCurrentFocus` 證 guest dispatch 活著；活著就查 host→gRPC。**Session 106 根因＝gRPC 埠被 orphan/舊 listener 搶**：固定 5554→8554 且 `m_grpcInput` 恆非 null（ADB fallback 死碼），若別的行程占了 8554（Windows 具體位址勝 wildcard），POST 誤路由＝點擊全丟。查 `netstat -ano | findstr :8554` 是否非本 emulator 占用；正解＝每次啟動挑驗證過空閒的 console/adb/**gRPC** 埠（`start-chimera.ps1` 已自動挑，勿硬寫 5554） |
