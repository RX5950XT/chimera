...
| 畫面有但點擊沒反應 | 普通點擊走 emulator gRPC `sendTouch`（fire-and-forget POST），顯示走獨立的 shared texture＝**畫面在不代表輸入通，反之亦然**。歷任根因（S106 埠被搶、S107 -Fast 顯示凍結）**皆已否證**（S108），原始症狀 0 次受控重現。拆帳法：① `CHIMERA_GRPC_INPUT_DIAG=1` 看每個 POST 的 netErr/http/grpc-status；② 輸入端到端＝PostMessage WM_LBUTTONDOWN 到 chimera-ui HWND + guest `mCurrentFocus` 行為 oracle；③ 顯示 liveness＝**三指標隨時間對照**（emulator stderr `kVk GPU path (frame N)` vs host `CHIMERA_PERF total` vs 螢幕 BitBlt hash，主動驅動 guest 變化下看背離）——單張 byte-identical 截圖是 idle-static 陷阱，不能當凍結證據。`netstat` 查埠、挑埠邏輯（`start-chimera.ps1` 自動挑含 gRPC 埠）仍是有效防線 |
