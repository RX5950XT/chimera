...
- **日常互動可用性 verifier**（非 GL60 synthetic）：`powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\verify-interactive-ui.ps1 -Mode Fast -GuestVulkan -MeasureSeconds 25`（需先 `$env:CHIMERA_GUEST_VULKAN='1'`；verifier 會設 `CHIMERA_GUEST_VULKAN_HOST_SETUP=0`，自己管理 skiavk restart/re-gate，避免與 host setup race）。量測 Home→Settings→sustained scroll→app switch，輸出 `CHIMERA_DISPLAY`/`CHIMERA_INT`/`CHIMERA_INT_PRIO`。`adb input swipe` 只代表測試注入路徑，不能外推成實際滑鼠手感；一次性 host mouse-drag probe 曾量到 `guestMax=116.7 / render=57.4 / dup=0`，但該測法會搶使用者滑鼠，**禁止再用**，後續若要量 host input 必須新增不移動實體游標的 internal synthetic touch hook。
