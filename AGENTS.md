...
- One-click/custom runtime smoke:
  `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\start-chimera.ps1 -Fast -SelfTest`。合格條件：boot completed、`wm size=1920x1080`、Chimera Launcher screenshot 非黑（約數萬 bytes）、Settings 可 foreground、0 residual process；`-Fast` 會設定 `CHIMERA_GFXSTREAM_HEADLESS_SWIFTSHADER_ES=1`，讓一般 UI 避開 headless HOST core-profile shader mismatch。
...
- True Android 1080p/60 verifier：`powershell -NoProfile -ExecutionPolicy Bypass -File .\scriptserify-true-1080p60.ps1 -WarmupSeconds 15 -MeasureSeconds 120`。預設 `-RuntimeKind Gfxstream`，必須使用 custom shared texture runtime；stock gfxstream runtime 正確結果是 fail，不可當達標。合格條件：先用 ADB screencap 證明目標 app foreground 且 1920×1080 非黑可見，再量 post-warmup steady gate `min ≥ MinEffectiveFps(57)` + `avg ≥ MinAvgEffectiveFps(59)` + `dup ≤ 5%`；post-warmup `effective<=0` 直接 fail，不能過濾。Session 89 嚴格 120 秒 PASS：`effective_fps_min=59.9 / avg=60.0 / dup=0`、`GPU-direct D3D11 import OK`、`path=GPU-direct`、raw/GL fallback=0。注意：60 FPS 需 guest 連續渲染，push-based idle/boot 低 FPS 屬正常。
- **日常互動可用性 verifier**（非 GL60 synthetic）：`powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\verify-interactive-ui.ps1 -Mode Fast -MeasureSeconds 25`。量測真實互動（Home→Settings→sustained scroll→app switch），輸出 `CHIMERA_DISPLAY`/`CHIMERA_INT`/`CHIMERA_INT_PRIO`：display path 分類、per-segment guest/stream/render/dup + bottleneck、emulator priority/CPU%/helperSpawns。`-Mode Stock`=baseline（永不宣稱 60）、`-Observe`=手動觀察並耳聽音訊、`-Priority idle|below_normal|normal` 量 audio↔FPS trade-off。`-GuestVulkan`（需先 `$env:CHIMERA_GUEST_VULKAN='1'`）把 HWUI 走 skiavk on NVIDIA，guest throughput ~2×（仍 push-based ~20fps）——此 flag 維持 **opt-in**（單獨無使用者好處、會影響未驗的 stock 日常路徑，見 CONTEXT Session 94）。Fast 一般 UI 實測 `effFps≈20`、path=gpu-direct、瓶頸 guest cadence。baseline/observe/pass 退出碼 0、失敗 throw（nonzero）。共用 harness 在 `scripts/ChimeraVerifyCommon.ps1`（兩支 verifier dot-source）。
- emulator priority / 互動資源 env：`CHIMERA_INTERACTIVE_PRIORITY`（`idle|below_normal|normal`，預設 `below_normal`）、`CHIMERA_INTERACTIVE_CPUS`/`CHIMERA_INTERACTIVE_RAM_MB`（預設 4/4096，但 normalizer floor `>=4/4096`）。`CHIMERA_DISPLAY` log 一行印實際 path/priority/cpus/ram。
...
| Emulator shared texture producer 接線 | **Session 89 嚴格可見 120 秒 PASS**：modern gfxstream `postImpl()` headless 分支在 `bridge.isEnabled() && bridge.isDirectVkReady()` 時走 `postFrameDirectGpu`（`colorBuffer->borrowForDisplay(kVk)` → Vulkan `recordCopy` GPU blit → D3D11 NT shared texture via Vulkan external-memory import），完全不走 `m_onPost` / `readback()` / staging `UpdateSubresource`；kVk borrow 失敗（GLES-backed 一般 UI）才退 `chimeraPublishFrameToD3D11Texture`。legacy `libOpenglRender` 的 `ChimeraSharedTextureBridge` hook 仍是 EmuGL 路徑（HAXM/WHPX 缺席而擱置）。`GpuFrameBridge` CPU pixels callback 太晚 |
...
| GPU-direct D3D11 shared texture path | 現況 `postFrameDirectGpu` 已是 D3D11 shared handle 經 `VkImportMemoryWin32HandleInfoKHR` 匯入成 VkImage，直接 `recordCopy`/GPU blit 到 shared texture；Session 89 120 秒可見 verifier PASS（min 59.9 / avg 60.0）。後續優化重點不是再回到 staging path，而是 keyed mutex/多開/更高解析度穩定性與減少診斷 log 干擾 |
