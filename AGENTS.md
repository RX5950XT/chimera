...
- True Android 1080p/60 verifier：`powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\verify-true-1080p60.ps1`。預設 `-RuntimeKind Gfxstream`，必須使用 custom shared texture runtime；stock gfxstream runtime 正確結果是 fail，不可當達標。classic EmuGL 只能用 `-RuntimeKind EmuGL` 明確驗證，不能跑 modern Android 34 Play image 時不可當完成證據。**Session 85 起此 verifier 會 PASS**：它 build/install `tools\chimera-gl60-smoke`（GLES 連續渲染 app）→ `-WarmupSeconds`（預設 30s）排除 app 冷啟 transient → 量 steady gate `min ≥ MinEffectiveFps(57)`（容許 windowed jitter）+ `avg ≥ MinAvgEffectiveFps(59)` + `dup ≤ 5%`。實測 `result=pass`（min 59.6–59.8 / avg 60.0 / dup 0，1920x1080，avgMs ~16.2ms）。注意：60 FPS 需 guest 連續渲染，push-based idle/boot 低 FPS 屬正常，不是 verifier 退步。
