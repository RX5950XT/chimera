...
- **Chimera UI shmem 整合 CONFIRMED**：以 `CHIMERA_EMULATOR_PATH` 指向 github runtime 啟動 Chimera UI 後，InstanceManager 自動啟用 shmem；開機動畫期間真實達到 `guest=16.3 stream=16.3 render=15.4 effective=15.4 FPS`；靜止 Home 後 0 FPS 為 push-based 正常行為。
- **下一步**：跑遊戲場景測量 60 FPS shmem；或新增遊戲場景 shmem verifier（要求 >= 30 FPS）。
