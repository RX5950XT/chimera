...
- **shmem 吞吐量 CRITICAL FINDING**：合成 56.7 FPS producer → Chimera 消費 `guest=stream=render=50.6 FPS`（`dupPct=0`，`avgMs=19.3ms`）。shmem 基礎設施上限 50-56 FPS；16 FPS 瓶頸是 Android Settings 複雜 UI 渲染，不是 shmem。**3D 遊戲預期 50-60 FPS**（GPU 主導，VCPU 只提交 draw call）。
- **Priority 測試**：BELOW_NORMAL `effective=15.9 FPS`；NORMAL 反而只有 8.7 FPS（執行緒競搶+stall）；BELOW_NORMAL 最佳。
- **下一步**：① 遊戲 APK 驗證真實 3D FPS；② D3D11 shared texture（GPU-to-GPU，無 CPU readback，理論 60+ FPS）。
