...
**第二個日常路徑 bug（使用者關閉視窗時發現）**：使用者按 X 關閉後 snapshot 檔案 mtime 完全未動＝graceful save 沒發生。根因：`adbPathForConfig` 從 `emulatorPath.parent².../platform-tools/adb.exe` 推路徑——`-Fast` 的 emulatorPath 是 `build\chimera-gfxstream-runtime\emulator.exe`，推出的 `build\platform-tools\adb.exe` 不存在→`runAdbEmuCommand` 回 false→`stop()` 落到 TerminateProcess＝unclean exit＝**snapshot 永不保存＋既有 snapshot 失效→`-Fast` 日常使用永遠 34s 冷開**（P1 的 7.5s 只在 verifier 的外部 `adb emu kill` 下成立過）。修法：`adbPathForConfig` 先試 emulatorPath 推導、不存在則 fallback `sdkRootForConfig()/platform-tools/adb.exe`（sdkRootForConfig 已涵蓋 avdHome 兄弟目錄與 ANDROID_SDK_ROOT/ANDROID_HOME）。E2E `pass-close-saves-quickboot`：launch1 冷開 36.8s（上次 unclean 的後果現形）→ **WM_CLOSE（＝按 X）** → 兩 process 60s 內乾淨退出、snapshot.pb mtime 前進（04:27:00→04:27:38）→ launch2 **8.5s** 快載 alive。unit 24/24。

**gate #2（一般 UI 60fps）定性更正＋ES3 否證**：候選 C（GLESDynamicVersion＝guest ES3）A/B 實測**零收益**——baseline effFps 54.3/effMin 44.5 vs ES3 52.9/41.5（變異內略差），guest 兩者皆 60fps；全程顯示健康＝**S109「ES3 弄壞 -Fast」正式否證**（當年凍結＝RefCountPipe bug 誤歸因）。關鍵觀察：一般 UI scroll 下 guest 已 60、限制在 host stream/render ~53-54＝**below_normal priority 的音訊取捨**（S104：normal 可 59-60）——SwiftShader 只在重 fill 是牆且 Vulkan 遊戲已繞過→深水候選（ANGLE draw AV／CompositorVk）對日常 UI 收益上限低，暫停深挖；要 60 的使用者用 `-InteractiveFirst`。實驗法＝user-level `~/.android/advancedFeatures.ini`（全域，測完必刪，已清）。

**教訓**：① verifier 必須跑出貨 flavor（stock-only 驗證蓋不住 `-Fast` 預設）；② guest brick 時 `adb shell` 永久 hang——所有等待迴圈的 adb 呼叫要包 timeout（Start-Job + Wait-Job）；③ 「adb devices 回 device」不等於 guest 活著；④ **「從某路徑推導另一工具位置」的 helper 在路徑來源多樣化（custom runtime）後會靜默失效**——失敗要 fallback 而非回傳空，且關鍵副作用（存檔）失敗不可無聲。
