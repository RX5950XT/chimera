...
- **cross-DLL ABI 修法「全部 feature 預設 false、需要的再 force-enable」是一筆會選擇性爆炸的債**：EglOnEgl/Vulkan/GlDirectMem/QueueSubmitWithCommands 都因「馬上壞」被補上，`RefCountPipe` 因為「幾乎能動」潛伏——guest gralloc 走 pipe 模式（不送 rcOpen/Close），host 卻跑 legacy refcount 對不參與的 guest 亂記帳（post-O create refcount=0、每次 headless post 瞬時歸 0→10s delayed-close；活躍時 markOpened 每幀救回、一靜止就過期）。**Rule**：接手這類「defaults false + 白名單 force-enable」的 ABI workaround，要把 emulator 側 advancedFeatures.ini 裡 on 的每一項逐一過帳「guest 行為依不依賴它」；「跑起來正常」不代表帳是平的，只代表還沒踩到結帳點。修法＝`m_refCountPipeEnabled` force true（kill switch `CHIMERA_GFXSTREAM_NO_REFCOUNT_PIPE=1`）。
- **同一 build 加 kill switch 做 A/B 是最強的因果證明**：fix build 3/3 GREEN（rcFBPost=postImpl=kvk 全程 lockstep、seq 1200）；同一 build 設 kill switch 2/2 RED（idle 後 rcFBPost/postImpl 照走、seq/kvk 凍結）。比「改前 vs 改後兩個 build」乾淨（排除 rebuild 差異）。**Rule**：修 gfxstream 這類重建成本高的元件，修法一律帶 env kill switch——同時買到 A/B 能力與現場回退能力。
- **判停更要三個計數器分層：rcFBPost（guest 指令到達）→ postImpl（進 FrameBuffer）→ published sequence（發佈成功）**。killswitch RED 顯示前兩者前進、第三者凍結＝掉在 publish 段；若 rcFBPost 也停＝guest 側。搭配 guest screencap/focus/wakefulness 即可四象限定位。**Rule**：鏈式管線的 liveness 診斷，每一跳一個 throttled 計數 log（1/60/%600），沒有計數的跳就是盲區（postImpl 的靜默 FAIL 曾是唯一無 log 的失敗點）。
- **診斷用直開 emulator 時 `-gpu swiftshader_indirect` 會把 VK ICD 一起拉成 SwiftShader**→bridge 的 D3D11 import 走到 null fn ptr＝RIP=0 崩潰。生產是 `-gpu host`（headless GLES 由 DLL 內部退 SwiftShader、VK 留 NVIDIA）。**Rule**：複製生產環境時 `-gpu` 模式照抄 `configs/instances.json` 的 `graphicsRenderer`（host），別想當然用 swiftshader_indirect（S80 教訓的另一面）。
- **guest 螢幕會睡（stay_on 沒生效時）＝另一個「畫面停格」擬態**：直開 emulator 無 chimera 的 `stay_on_while_plugged_in` 設定時，idle ~60s 螢幕熄→posts 停、`am start` 也叫不醒 rendering＝跟停更長得一樣。repro 腳本要先 `svc power stayon true`。**Rule**：診斷「畫面不動」先查 `dumpsys power` 的 `mWakefulness`，排除螢幕睡眠這個便宜解釋。
- **啟動「鈴聲」＝guest 充電提示音**（emulator 每次 boot 都「剛插上 AC」）＋keyguard 解鎖音；修在 `applyGuestFirstBootSetup`：`charging_sounds_enabled=0`（secure+global）、`lockscreen_sounds_enabled=0`、`sound_effects_enabled=0`。
- **殘餘不確定（誠實）**：killswitch RED 中「post 完成但 publish 停、handle lookup 不 miss」的最後一段微觀機制（疑 guest 以同 resource id 重建 CB／外層 else 的 CPU 發佈靜默失敗）未逐指令釘死；但同 build A/B 已證因果，且 RefCountPipe=true 是 emulator/guest 實際組態的正確對齊（stock 模擬器本來就這樣跑）。
