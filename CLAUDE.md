...
**Current runtime gate**: stock emulator HWND/window capture 與 native embed 都是 unsafe diagnostics；除了 unsafe CLI 參數外還必須設 `CHIMERA_ALLOW_UNSAFE_VISIBLE_EMULATOR_WINDOW=1`，且由同次 Chimera 啟動建立內部 `CHIMERA_VISIBLE_EMULATOR_DIAGNOSTICS_SESSION=1`，才允許外露原生視窗。正常啟動強制 headless / `-no-window`，若 emulator/qemu tree 仍外露可見 HWND 就立即終止。根目錄 `start-chimera.cmd` 現在預設走最快可用路徑：`start-chimera.ps1 -Fast -InteractiveFirst` → custom gfxstream shared texture + `CHIMERA_GUEST_VULKAN=1`（**只**加 `-feature Vulkan`，Vulkan app 直達 NVIDIA）+ animations=1；`-Stock` 才強制回 stock gRPC 慢路徑。**Session 100 修復 `-Fast` 黑屏根因並禁止 skiavk UI 切換**：舊 host 端 boot 後 `setprop debug.hwui.renderer=skiavk`+`stop/start` 在此 playstore user image（無 root）restart 必失敗（"Must be root" 被吞），半套用（HWUI Vulkan + SF SkiaGL/SwiftShader 跨裝置無法取樣）讓 setprop 後啟動的 app 視窗全黑=「狀態列有、中間黑」；probe 亦證 `-systemui-renderer skiavk` 只翻譯 hwui prop、SF 永遠無法切 Vulkan → **skiavk UI 在此 image 結構性不可行，勿再嘗試**（歷史「GuestVulkan 對照/hwui=skiavk active」數字是採樣 pre-setprop GLES process 的假證據）。啟動時間：`boot_seconds≈33`、`visible_home_seconds≈49`（Session 100 SelfTest；Session 101 量到 87/103s，連續多次 boot 的環境噪音成分待重測）；boot 期間 QML placeholder 由 `AndroidControls.bootReady` 撐到 boot 完成，launcher 以 md5 比對跳過重複 reinstall。**Session 101 重大更正：`-Fast` host 視窗從 Session 85 起顯示的一直是零幀（黑）**——三層 bug 疊加（compose 不標 `mGlTexDirty` → kVk image 從未被寫入；`OPAQUE_WIN32` 匯入無 aliasing；consumer 無 AcquireSync），歷來 gate 只驗 guest ADB screencap + host counters 所以全綠。三層已修（`flushFromGl+invalidateForVk` 前置同步、`D3D11_TEXTURE_BIT`+dedicated import、GuestDisplay keyed-mutex acquire+私有副本），SelfTest 新增 host 視窗像素 gate `host_window_nonblack_pct` 防再犯；實測 host 視窗顯示真實 Android UI、互動中有效 43 FPS。歷史「Session 89 嚴格可見 gate PASS 60fps」數字需重新定性：當時 ADB screencap（guest 端）+ host counters 全綠，但 host 視窗實際是黑的，60 量的是零幀 blit 節奏；修復後 GLES 內容每幀多付 GL readback+VK upload（SwiftShader CPU），真實 gl60 數字見 Session 101 記錄。frame path 是 source-patched `postFrameDirectGpu`：D3D11 NT shared texture 由 Vulkan external-memory import（`D3D11_TEXTURE_BIT`+dedicated），guest 內容經 GL→VK 同步後 GPU `recordCopy` blit 到 shared texture。另修 emulator idle 自殺：`-idle-grpc-timeout 300` 已移除（shared-texture 顯示不走 gRPC，黑屏無輸入 300s 即自殺=「等多久都黑」的第二半）。一般 Android UI：`verify-interactive-ui.ps1 -Mode Fast -GuestVulkan` standalone 已補設 `CHIMERA_GUEST_VULKAN=1`，detached baseline 實測 sustained scroll `guestFps=48.6 / streamFps=40.5 / renderFps=40.3 / effFps=40.3 / dup=0 / bottleneck=render`；先前約 20fps 的 GuestVulkan 結論是 verifier 未真開 `-feature Vulkan`、skiavk 退 SwiftShader 的污染數字。adb-swipe verifier 只能代表測試注入路徑；一次性 host mouse-drag probe 量到 production input path `guestMax=116.7 / render=57.4 / dup=0`，但該測法會搶實體滑鼠，使用者要求後禁止再用。`start-chimera.ps1 -Fast -InteractiveFirst -SelfTest` 已驗證 1920×1080、Settings foreground `interactivity=ok`、0 residual。注意：60 FPS 證據仍須分清 workload；push-based idle Home 低 FPS 屬正常。verifier/self-test 會透過 `ChimeraVerifyCommon.ps1` 自動挑空的 emulator console/ADB port pair、拒絕 odd console port，且 cleanup 只清 cmdline-filtered Chimera process tree，避免本機服務占用 `console+1` 或誤殺非 Chimera emulator；post-warmup `effective<=0` 直接 fail，不能過濾成假 PASS。Chimera 走 fork/改 Android Emulator + gfxstream/QEMU runtime，不是完整重寫 Android VM，也不允許正式路徑多開原生 Android Emulator 視窗.
...
| 真 60 FPS 互動流暢度 | RESOLVED（連續渲染 + 一般 UI 互動）— Session 85：`chimera-gl60-smoke` 連續渲染 `min 59.8/avg 60.0/dup 0`。**Session 99：general-UI 60 達成（真實輸入路徑）**——`verify-interactive-ui.ps1 -Mode Fast -GuestVulkan -SyntheticScroll` strict gate `result=pass-gpu-direct-60`：`effFps 59.6/effMin 58.1/guest 60.1/dup 0`，重現 2×；gl60 非回歸 `min 59.9/avg 60.0`。**瓶頸是 host 端非 guest gfxstream**（推翻 15-session 判斷）：`QSG_RENDER_LOOP=threaded` 解 host→guest backpressure（guest 55.9→60.1）+ present timer 16ms→200ms 解 GUI-thread contention（stream/render 54→59.6），零 DLL 重建。量測走 `CHIMERA_SYNTHETIC_SCROLL` production-path fling 注入器（不搶實體滑鼠）。**Session 94 GLES 重負載探針**（另一維度）：gl60 `-HeavyIterations 96` → `6.6`，重 GLES fill 瓶頸是 guest SwiftShader 軟體填色，HW 重遊戲須走 guest Vulkan→NVIDIA（連續渲染 60 未逐一量）|
...
| `-Fast` 載入後中間畫面全黑（狀態列可見） | RESOLVED — Session 100：根因是 host 端 skiavk 半套用（setprop 生效、`stop/start` 在 playstore user image "Must be root" 靜默失敗 → HWUI Vulkan + SF SkiaGL 跨裝置無法合成）。移除 skiavk 切換（此 image 結構性不可行）；`CHIMERA_GUEST_VULKAN` 只剩 `-feature Vulkan`。SelfTest 補 screenshot 內容 gate 防再犯 |
| 載入慢 + boot 期間裸黑無回饋 | RESOLVED — Session 100：移除 skiavk restart/12s grace、launcher md5 跳過重複 reinstall → `boot≈33s`、`visible_home≈49s`（原 ~80–110s）；QML placeholder 以 `AndroidControls.bootReady` 撐到 boot 完成。Quick Boot（更快）維持 opt-in |
| 背景手把輸入漏進 guest（guest 前景 app 隨機被 HOME/BACK 關掉） | RESOLVED — Session 100：`GamepadManager` XInput 全域輪詢無條件轉發，使用者在別的遊戲按手把會打進 Android（sidecar 實錄凍結瞬間 guest 前景變 launcher）。修：`gp.poll()` 前檢查 `QGuiApplication::applicationState()==ApplicationActive`（BlueStacks 同款 focus-gate 行為） |
...
- **競品研究**：`docs/references/competitor-emulator-smoothness.md`。Chimera host pipeline 已與 BlueStacks/LDPlayer/MuMu 同級；當時一般 UI ~20fps 落差主要是 guest 繪製仍跑在 SwiftShader（軟體）。Session 96 修正 standalone `-GuestVulkan` verifier 後，真 Vulkan sustained scroll 已量到 `effFps=40.3 / guestFps=48.6`；general-UI 全程 60 仍需 guest cadence / host render R&D。
...
- **⚠ Session 100 更正**：本段的 skiavk 部分對 production 不成立。production 的 setprop+`stop/start` 在 playstore user image 從未成功（"Must be root" 被吞），半套用狀態＝黑屏 bug；「hwui=skiavk active/~2×」數字採樣的是 pre-setprop GLES process。skiavk UI 切換已從 production 移除並禁止再加（見 Session 100）。

## 2026-07-02 — Session 101 — `-Fast` host 視窗黑屏三層根因全修 + emulator idle 自殺修復 ✅

- **使用者回報**（Session 100 修復後仍黑）：啟動等很久還是黑畫面、有效 FPS 0。
- **重現與分層取證**：guest ADB screencap 75KB 正常、producer `published sequence` 跳動、host `total` 增長、host 視窗 PrintWindow 中心像素 (2,5,4)=黑——「counters 全綠、像素全黑」。三支獨立 probe 定案：`texprobe.exe`（D3D11 端讀 shared texture，有/無 AcquireSync）、`vkinterop.exe`（隔離 Vulkan→D3D11 匯入/寫入/跨 process 矩陣）、bridge 內 `debugReadbackSharedImage`（VK 端讀回 mImage）。
- **Bug A：shared texture 從 Session 85 起發佈的一直是零幀**，三層疊加：
  1. **GL→VK 同步缺失（最深層）**：HWC compose 由 host GL（CompositorGl）寫 target CB，從不標 `mGlTexDirty`（只有 `rcFlushWindowColorBuffer` 會標）→ `invalidateForVk()` 恆 `exit=clean` → `borrowForDisplay(kVk)` 的 VK sibling image 從未被寫入 → blit 複製零。修：headless post 分支 borrow 前 `colorBuffer->flushFromGl(); colorBuffer->invalidateForVk();`（VK-backed 內容兩者皆 no-op、保 zero-copy）。VK readback 實證修前 `nonzero=0/120` → 修後 `120/120`。
  2. **Vulkan import 無 aliasing**：D3D11 NT handle 用 `OPAQUE_WIN32_BIT` 匯入（無 dedicated info）→ NVIDIA 回 VK_SUCCESS 但寫入進私有記憶體。修：`VK_EXTERNAL_MEMORY_HANDLE_TYPE_D3D11_TEXTURE_BIT` + `VkMemoryDedicatedAllocateInfo`（vkinterop 隔離實證 OPAQUE=丟失、D3D11_TEXTURE=可見）。
  3. **Consumer 無 AcquireSync**：keyed-mutex texture 跨 process 不 acquire 讀=零（矩陣實證）。修：GuestDisplay 每新 sequence `AcquireSync(0)`→`CopyResource` 到私有 texture→`ReleaseSync(0)`，QSG 取樣私有副本；注意 `WAIT_TIMEOUT(0x102)` 能過 `SUCCEEDED()`，須 `==S_OK`。
- **Bug B：emulator idle 自殺**：`VirtualMachine.cpp` 傳 `-idle-grpc-timeout 300`；`-Fast` 顯示不走 gRPC → 黑屏無輸入 300s → `IdleInterceptor` 優雅關機（`Idled to long, shutting down`）→ 使用者「等多久都黑」。stock 路徑 getScreenshot 輪詢永不 idle、verifier 全程注入 input，故從未踩到。修：移除旗標（Job Object kill-on-close 已管 orphan）+ `grpcEnabledNeverRequestsIdleShutdown` unit test。
- **Gate 漏洞修補**：歷來所有「可見」gate 都驗 guest ADB screencap + host counters，皆不經「shared texture 內容→host 視窗」→ 三層 bug 潛伏 15 session。新增 `ChimeraVerifyCommon.ps1::Get-HostWindowPixelStats`（PrintWindow PW_RENDERFULLCONTENT + 中央區取樣）並接入 SelfTest（gate `host_window_nonblack_pct>=5`）。
- **patch script drift 教訓**：modern tree 的 headless 段是前 session 手改（patch 對應段死碼、靜默跳過）；本輪 tree+script 同步修，新增段用 `Replace-Text`（含 already-applied check）。診斷留存並 codify：bridge `debugReadbackSharedImage`（每 240 幀）、`invalidateForVk` exit-path log（每 600 次）。
- **驗證**：ctest 23/23 PASS；`start-chimera.ps1 -Fast -InteractiveFirst -SelfTest` **PASS**（`screenshot_nonblack=100%`、**`host_window_nonblack_pct=100.0` 新 gate**、interactivity=ok、0 residual）；host 視窗實截顯示 Settings+IME、互動中有效 43 FPS。
- **誠實邊界**：歷史 GPU-direct 60fps 數字（S85/89/99）量的是零幀 blit 節奏，需以修復後數字重新基準（GLES 內容每幀多付 SwiftShader GL readback + VK upload）；`boot_seconds` 本輪 87s vs S100 33s，連續 boot 環境噪音成分待空機重測。

## 2026-07-02 — Session 100 — `-Fast` 黑屏根因修復 + 載入加速 ✅

- **使用者回報**：雙擊載入直接黑屏（側 UI 正常、中間模擬畫面全黑）、載入慢。
- **根因（重現＋隔離 probe 雙實證）**：Session 95 加的 host 端 skiavk 切換在 `google_apis_playstore` user image（`ro.debuggable=0`）上 `stop`/`start` 回 **"Must be root"** 被 `runAdbShell` 靜默吞掉 → SF 永遠 SkiaGL；但 `debug.hwui.renderer=skiavk` 有寫入 → setprop 後首次 HWUI init 的 process（launcher 被 `pm install -r` 重啟）走 Vulkan → host 上 NVIDIA VK surface 無法被 SwiftShader-ES 合成取樣 → **app 視窗全黑**；pre-setprop 的 SystemUI 可見 → 「狀態列有、中間黑」。「偶發 black-boot」假象＝process 啟動時序決定可見與否。probe `-systemui-renderer skiavk`：init 只翻譯 hwui prop、不翻 renderengine → `Pipeline=Skia (Vulkan)` 實證下 home/Settings screencap 全空白 → **skiavk UI 此 image 結構性不可行**（root restart 無、boot-time prop 無；唯一未定案第三路＝`setprop ctl.restart surfaceflinger`，屬 R&D）。
- **修復**：main.cpp 移除 `applyGuestVulkanHardwareUi()`＋poller skiavk/12s grace；`CHIMERA_GUEST_VULKAN`＝只加 `-feature Vulkan`；launcher 安裝改 md5 比對跳過重複 reinstall/force-stop；QML placeholder 綁新 `AndroidControls.bootReady`（boot 完成才撤，不再第一張黑幀就消失）；SelfTest 補 screenshot 內容 gate（bytes≥20000/nonblack≥10%/spread≥40）；verify-interactive-ui 移除 skiavk 虛構流程；`Invoke-CheckedTool` 修 native-stderr（javac Note）在 redirect 下變 terminating error 的 harness bug。
- **skiavk 三路全定案死路**（probe 實證）：① `stop/start`＝"Must be root"；② `-systemui-renderer`/boot prop＝init 不翻譯 renderengine；③ `setprop ctl.restart surfaceflinger`＝SELinux denied（`scontext=u:r:shell:s0 permissive=0`、SF pid 不變）。
- **第二個真 bug：背景手把輸入漏進 guest**（60fps 驗證時抓到）：使用者在主螢幕玩 P5R（手把），`GamepadManager` XInput（system-wide）60Hz 輪詢無條件轉發 → 手把 B/HOME 打進 Android 把前景 app 切到背景 →「guest 隨機停止渲染」。sidecar 凍結瞬間取證（guest 前景=launcher、gl60 process 活、螢幕 Awake）。修：`gp.poll()` 前 `QGuiApplication::applicationState()==ApplicationActive`（BlueStacks 同款 focus-gate）。修後 sidecar 實錄 gl60 全程前景。
- **驗證環境改進**：chimera-ui 啟動即讀 `CHIMERA_VERIFY_WINDOW_ORIGIN`（"x,y"，負值=左側副螢幕）setPosition——測試視窗**從第一幀就在副螢幕**（先前 boot 階段停主螢幕預設位置蓋住使用者遊戲、被關掉 → run 中斷）；`Ensure-HostWindowVisible` 對 exited process 的 `MainWindowHandle`（ETS null→`[IntPtr]$null` cast）try/catch 防禦；env 已存 User scope `-2520,45`。
- **驗證**：build PASS；ctest 23/23；`-Fast -InteractiveFirst -SelfTest` PASS（`boot_seconds=33`、`visible_home_seconds=49`、screenshot 76,219B/nonblack 100%/spread 716、interactivity=ok、0 residual）；載入從舊 ~80–110s（含 restart+grace+reinstall）→ **~40-49s 可見 home**。
- **60fps 現況（誠實邊界）**：與使用者 AAA 遊戲（P5R）同跑時：gl60 量測窗內連續 **150s 60.0fps**（min 59.5/dup 0）但嚴格 gate 被遊戲負載尖峰打穿（min 8.6）；互動 verifier 完整跑完 `path=gpu-direct / scroll eff 49.1 / dup 0`（`fast-ui-visible-not-60`）。空機嚴格 PASS 證據＝Session 89/99（同 render path，本輪未動）；空機時重跑兩個 verifier 即可重確認。

---
*Updated: 2026-07-02 — Session 101（**`-Fast` host 視窗黑屏三層根因全修**：① HWC compose 不標 `mGlTexDirty` → kVk sibling image 從未被寫入 → headless post 補 `flushFromGl()+invalidateForVk()`；② D3D11 NT handle `OPAQUE_WIN32` 匯入無 aliasing → 改 `D3D11_TEXTURE_BIT`+`VkMemoryDedicatedAllocateInfo`；③ consumer 無 AcquireSync 跨 process 讀到零 → GuestDisplay keyed-mutex acquire+私有副本〔`WAIT_TIMEOUT` 過得了 `SUCCEEDED()`，須 `==S_OK`〕。**shared texture 從 Session 85 起發佈的一直是零幀**，歷來 gate 只驗 guest ADB screencap+host counters 所以 15 session 全綠；SelfTest 新增 `Get-HostWindowPixelStats` host 視窗像素 gate。**Bug B：`-idle-grpc-timeout 300` 移除**——shared-texture 顯示不走 gRPC，黑屏無輸入 300s emulator 自殺=「等多久都黑」的第二半；+unit test。三支 probe 定案法：texprobe〔D3D11 端±acquire〕、vkinterop〔隔離匯入矩陣〕、bridge VK readback。驗證：ctest 23/23、SelfTest PASS〔host_window_nonblack=100%、interactivity ok、0 residual〕、host 視窗實截 Settings+IME 有效 43 FPS。誠實：歷史 GPU-direct 60fps=零幀 blit 節奏，需重新基準）；Session 100（`-Fast` 黑屏根因修復：skiavk 半套用〔setprop 生效、stop/start "Must be root" 靜默失敗〕→ 移除 skiavk UI 切換〔三路全實證死路：root restart / boot-prop 不翻譯 / ctl.restart SELinux denied〕；載入 ~80–110s→~40-49s〔無 restart/grace、launcher md5 跳過 reinstall〕；placeholder 綁 bootReady；SelfTest 補像素內容 gate；**第二 bug：GamepadManager XInput 全域輪詢漏使用者手把輸入進 guest〔玩 P5R 按 B=Android BACK 關掉前景 app〕→ focus-gate 修復**；chimera-ui 啟動即讀 CHIMERA_VERIFY_WINDOW_ORIGIN 開副螢幕；`Invoke-CheckedTool` EAP=Continue、`Ensure-HostWindowVisible` exited-process 防禦；60fps：與 AAA 遊戲同跑量到窗內連續 150s 60.0 + scroll eff 49.1/dup 0，嚴格 gate 需空機重跑〔Session 89/99 空機 PASS、render path 本輪未動〕）；Session 99（/goal 修復所有問題：sibling-grep 補洞。C++ production 複查 5 類已修 bug class〔硬寫 port、`CreateSharedHandle` flag、`waitForExit` double-close、env `stoi/atoi` 解析、tap press/release pairing〕全乾淨無新 bug；修 2 個 PowerShell harness sibling：① `run-proxy-smoke-test.ps1` 殘留掃描仍全域 `Get-Process -Name emulator,qemu-system*` + `$rp.Kill()`〔Sessions 96-97 修主 verifier 時漏的 R&D probe，會殺到 Android Studio/BlueStacks/其他 instance〕→ dot-source `ChimeraVerifyCommon.ps1` 改用 cmdline-filtered `Get-ChimeraProcesses`〔本來就 `taskkill /T` 過自己 PID tree，全域補刀只殺到別人的〕；② `verify-quick-boot.ps1` 刪掉 shadow 共用版的 local `Stop-/Wait-NoChimeraProcesses` 冗餘 re-def。**再實修 Session 98 deferred 的 2 個 finding**（goal hook 要求不留 deferral；重判後可正當修）：③ `AudioBridge.cpp` WASAPI forced-format bug 比原判嚴重——`GetMixFormat` 多回 `WAVE_FORMAT_EXTENSIBLE`(cbSize=22)、code 改 `wFormatTag=IEEE_FLOAT` 沒重設 `cbSize` → shared-mode `Initialize` 對任何 rate 都失敗（"Audio (WASAPI)" 其實全壞）；修 `cbSize=0`+`AUDCLNT_STREAMFLAGS_AUTOCONVERTPCM|SRC_DEFAULT_QUALITY`，新測 `forcedNonNativeRateInitializesViaAutoConvert` **實機 RED(QSKIP「無 endpoint」，被 bug 偽裝)→GREEN(PASS)**；④ gfxstream `mStagingBuffer` 死分支 grep 證明無 `vkCreateBuffer` 創建/無非 null 賦值 → 移除 recordCopy 死 if-branch+reset 死 destroy+`.h` 死 member（靜態可證、verbatim 流入下次 DLL rebuild）。完整 Release build PASS、`ctest -LE integration` 23/23 PASS（test-audio-bridge 含新 GREEN 方法）。**深層路線 general-UI 60（使用者選「投入深層路線」）**：發現過去「~20fps not-60」大半是 **adb-swipe 量測污染**（adb-swipe GuestVulkan scroll 量到 `guestFps=0.0 dup=0.0`＝完全沒驅動 guest 重繪）。新增 `CHIMERA_SYNTHETIC_SCROLL`（main.cpp diagnostics-only 預設關）走 production 路徑 `InputBridge::onTouchPoint→gRPC sendTouch` 連續 fling，**不碰實體滑鼠**；`verify-interactive-ui.ps1 -SyntheticScroll`。**真實輸入路徑實測**先 `~53 eff/~56 guest`，再用**兩個 host-only 改動（零 DLL 重建）達成 general-UI 60**：① `QSG_RENDER_LOOP=threaded`（main.cpp）解 host→guest backpressure（guest 55.9→60.1）；② GuestDisplay present timer 16ms→200ms 解 GUI-thread contention（stream/render 54→59.6）。誠實 strict gate **`result=pass-gpu-direct-60`**（`effFps 59.6 / effMin 58.1 / guest 60.1 / dup 0`，**重現 2×**）；gl60 連續渲染非回歸 `min 59.9/avg 60.0`；ctest 23/23、0 residual。**根因翻轉**：general-UI 60 卡點是 host render-loop backpressure + present-timer contention，**非** 15 session 認定的 guest gfxstream frame-pacing（被 adb-swipe 量測污染遮蔽）。量測走 `CHIMERA_SYNTHETIC_SCROLL` production-path fling 注入器，不搶實體滑鼠；custom runtime 仍偶發 black-boot〔既有 intermittent〕）；Session 96（ultracode 14-agent 審查 workflow〔4 維 find → 對抗式 verify〕修 7 個 P2：`start-chimera.ps1` `-RequireSharedTexture` 加 fail-closed throw、`ConsolePort` 限 even、README `~60 headroom` 過度表述改誠實、`verify-quick-boot.ps1` 改 dot-source 共用 harness〔free-port + cmdline-filtered kill〕、`verify-interactive-ui.ps1 -GuestVulkan` 補設 `CHIMERA_GUEST_VULKAN=1`〔否則沒帶 `-feature Vulkan`、skiavk 假對照〕、`AGENTS.md` 0x0B VT char 還原。對抗式 verify 否證 3 假陽性〔boot gate robustness/`$LASTEXITCODE` 實機 exit 0/CLAUDE 歷史段〕。皆 robustness 非 FPS；ctest 20/20、Fast SelfTest PASS。Session 94：roadmap item 2/4 收尾。**item 2**：gl60 加 `-HeavyIterations` 重 GLES fill 探針〔1080p 全螢幕 plasma〕。light `min 59.2/avg 60.0/dup 0`〔60〕vs heavy96 `min 5.8/avg 6.6/dup 0`——同一 gpu-direct path 只加重 per-frame GLES fragment 即 60→6.6，瓶頸是 **guest GLES backend=host SwiftShader 軟體填色**非 post path。故 gl60 的 60 依賴 trivial fill；HW 重遊戲須走 guest Vulkan→NVIDIA，其**連續渲染** 60 仍未量〔無 Vulkan game-loop app，scope-cut〕。**item 4 GuestVulkan 預設化評估＝維持 gated**：證據 ctest 20/20、gl60+`CHIMERA_GUEST_VULKAN=1` `min 57.9/avg 59.7/dup 0`〔flag 不回歸 60〕、interactive `-GuestVulkan` `hwui=skiavk` active 無 crash `guestFps=21.7`〔~2× skiagl〕gpu-direct 0 residual〔flag 安全+robust〕；但**不翻預設**——`-feature Vulkan` 單獨無使用者好處〔skiavk 需 runtime props，未接進正常 boot〕、翻預設影響未驗的 stock 日常路徑、真正預設＝feature 非翻 flag。item 4 無 code 變更。**順手修** `verify-interactive-ui.ps1` baseline/observe/pass `return`→`exit 0`〔`finally` cleanup native 洩漏 `$LASTEXITCODE` 致 success 誤 exit 255；實測 `try` 內 `exit` 仍跑 finally〕。**md5 對帳**：部署 gfxstream DLL md5＝`c81d2092…`〔≠ Session 91 記錄；build 非 byte-deterministic，md5 非 gate，manifest buildId+runtime 60 才是〕。Session 93：stock gRPC `total=0` 根因＝**hardcoded capture port**〔`main.cpp` 建 capture 硬寫 `8554`，但 emulator gRPC port=`g_runtimeCfg.grpcPort=8554+console offset`；修法 capture 改用 derived port〕+ per-request `setTransferTimeout` 解耦 stall watchdog〔`CHIMERA_GRPC_REQUEST_TIMEOUT_MS` 預設 6000〕+ retry timer `hasInFlight()` gate；robustness 非 FPS〔stock 仍 ~4–17 FPS〕。Session 92：`PostWorker::block()` early-return 補 `scheduledSignal->signal()`〔macOS-only latent，Windows DLL byte-identical〕。Session 91：修 3 個 MSVCP140 future crash〔device_op_tracker/sync_thread/WorkerThread〕→ app HWUI Vulkan 硬體渲染、~2× guest throughput；`CHIMERA_GUEST_VULKAN=1` + `verify-interactive-ui.ps1 -GuestVulkan`）*
