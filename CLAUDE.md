...
---
*Updated: 2026-07-04 — Session 104：穩定 60fps 全鏈實測——逐幀否證「host vsync 量化」假設（省下無效 swapInterval 改動）；定調使用者一鍵配置（`-InteractiveFirst`=normal priority）已穩定 ~57–60（pass-gpu-direct-60/effMin 54），瓶頸是 GL→VK readback 架構 floor 非 host present；移除生產 post hot-path 每 4s 的診斷 GPU readback（`CHIMERA_GFXSTREAM_DIAG_READBACK` gate，預設 off）；使用者側最大平滑度＝144Hz 改 120Hz。前一輪 S103：載入畫面進度條 + 手勢列閃爍定調 host present pulldown（動畫關）。詳見 `CONTEXT.md`。*
