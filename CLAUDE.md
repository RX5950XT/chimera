...
| 真 60 FPS | PARTIAL — **Session 102 全鏈拆帳定案：~57fps 是 vsync frame-pacing boundary，非單一可修瓶頸**。host consumer 恆 0.1ms（最佳）、guest 34% CPU（非 compute-bound）；瓶頸＝每幀 glReadPixels+2 VK submit+wait 偶超 vsync。A/B 換 CPU-direct post 使 guest 升 60 但 effective 不變（位移 host present）。真 60 需 guest Vulkan-backed 內容（被 skiavk 牆擋）+ host present pacing。互動 scroll 不穩大半是 idle gap（內容不變非 stall）。S101「零幀」更正、S99 host 修、S94 GLES fill 牆仍有效 |
