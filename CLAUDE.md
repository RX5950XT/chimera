...
## 當前狀態（2026-07-04 / Session 107）

- **Session 107（本次）**：**「有畫面但完全無法點擊」真根因＝-Fast 顯示凍結，非輸入；S106「gRPC 埠被搶」被否證**。系統化拆帳：① 現場 netstat **乾淨無搶埠**、gRPC POST **3138 個全 `http200 grpc-status0`**（加 `EmulatorGrpcInput` 診斷日誌實測）＝S106 根因錯。② **輸入路徑證實完好、端到端**：真實 Windows `WM_LBUTTONDOWN`（PostMessage，不搶實體滑鼠）→`GuestDisplay::mousePressEvent`→`mapToGuest`→`onMouseButton`→`sendTouch`→**guest 開 SearchActivity**（focus 變）；`WM_NCHITTEST=HTCLIENT`、`WindowFromPoint`=本視窗、兩螢幕皆 96DPI＝排除 hit-test/DPI/前景/覆蓋。③ **真根因＝顯示凍結**：權威螢幕 BitBlt liveness——guest HOME→Settings（確變），但 **host 視窗像素 byte-identical（凍結）**；`CHIMERA_PERF total` 卡在 ~140、maxMs 8184ms、連續 20 次 adb swipe 也不動；emulator 端 `postFrameDirectGpu` 停在 ~frame 100/seq 240 後噴 `Failed to find ColorBuffer 65/34/67`＝**producer 在 ColorBuffer handle 失效後停止發佈新幀**（guest 續跑、輸入照進、凍結畫面永不顯示反應＝「有畫面像死當」）。④ **立即修法（使用者選「先切 stock」）**：`start-chimera.cmd` 一鍵預設 **拿掉 `-Fast`→走 stock gRPC 顯示**（實測 live、可正常點擊，~10-19fps）**且拿掉 `-InteractiveFirst`→回 below_normal 優先權**（順手修使用者回報的**開模擬器音樂雜音**＝normal priority 搶 host audio thread；stock 慢路徑用 normal 幾乎沒 FPS 好處）。⑤ **待修**：-Fast 的 ColorBuffer 生命週期凍結（deep gfxstream `frame_buffer.cpp` post 路徑，需重建 runtime）。教訓：**FPS≠輸入送達；「pass-gpu-direct-60」量的是 render 節奏、非點擊送達或畫面 liveness**——任何顯示宣稱必須用 host 螢幕 BitBlt 對 guest 變化做 liveness 比對。
...
| 問題 | 狀態 |
|------|------|
| 有畫面但完全無法點擊（看似無反應） | WORKAROUND — Session 107 更正：**S106「gRPC 埠被搶」根因是錯的**（netstat 乾淨、gRPC POST 全 200）。真根因＝**-Fast 共享貼圖顯示在開機 ~240 幀後凍結**（producer `postFrameDirectGpu` 停止發佈：`Failed to find ColorBuffer` handle 失效），guest 續跑、輸入照進、但凍結畫面永不顯示反應＝「有畫面像死當」。**輸入路徑證實完好**：真實 Windows WM_LBUTTONDOWN→GuestDisplay→sendTouch→guest 開 SearchActivity（端到端）；gRPC 3138 POST 全 `http200 grpc-status0`。權威證據＝螢幕 BitBlt liveness（HOME→Settings 但 host 像素 byte-identical）。**立即修法**：`start-chimera.cmd` 一鍵預設改走 **stock gRPC 顯示**（實測 live、可正常點擊）。**待修**：-Fast 的 ColorBuffer 凍結（deep gfxstream，需重建 runtime）｜工具：`EmulatorGrpcInput` 加 gRPC POST 失敗/座標診斷（`CHIMERA_GRPC_INPUT_DIAG`） |
...
| emulator/qemu 搶 host audio（音樂雜音） | PARTIAL — Session 107：一鍵 `start-chimera.cmd` **預設拿掉 `-InteractiveFirst`**（normal＝最傷音訊、stock 路徑又幾乎沒 FPS 好處）→ 回 `below_normal`（`CHIMERA_INTERACTIVE_PRIORITY` 預設）。要最省音訊加 `-AudioFirst`（idle/EcoQoS），要最順加 `-InteractiveFirst`（換音訊雜音）。helper 走 `LowInterferenceProcess`；調節桿＝priority + readback CPU |
