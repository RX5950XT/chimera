...
## 當前狀態（2026-07-10 / Session 112）

- **Session 112c——Quick Boot 跨組態 snapshot 污染 brick＋人類保真度點擊 gate**：使用者「你要確定現在是人類可以正常使用點擊的模擬器」→ 用真實 Windows 滑鼠訊息驗證時當場抓到 **P1 的驗證盲區**：`verify-quick-boot.ps1` 直接跑 `chimera-ui.exe`＝**stock runtime flavor**，其存的 `default_boot` 被一鍵 `-Fast`（custom gfxstream）載入 → **guest 整台掛死**（snapshot load 950ms「成功」但 adb shell 永久 hang、qemu 713s 只用 4.4s CPU、host total 卡 1）＝emulator 自身相容檢查看不到 DLL/env flavor 差異。① **隔離實驗**：`-Fast` 自洽性 3 階段（冷開 32.5s→載入 8.5s 活→再載入 8.5s 活）＝`pass-fast-quickboot-selfconsistent`，根因純粹是跨 flavor 污染。② **修法（匯聚點）**：`VirtualMachine.cpp` `invalidateQuickBootOnDisplayFlavorChange`——AVD 旁 `chimera_display_flavor.txt` marker（emulatorPath+CHIMERA_GUEST_VULKAN+SWIFTSHADER_ES），launch 時 flavor 變即刪 `default_boot`＝跨組態自動冷開，涵蓋所有啟動路徑（腳本/verifier/直開）。③ **A/B 驗證** `pass-flavor-marker-ab`：marker 缺失→清＋冷開活；stock 存→-Fast 載（原 brick 場景）→`Quick Boot snapshot invalidated` 記錄＋冷開活；同 flavor→8.6s 快載活、無誤清。④ **人類保真度 gate**（真實 `WM_LBUTTONDOWN` PostMessage→兩點探測建 host→guest 仿射映射→點 launcher「設定」）：round1 kernelTouch+focus 切換+host 幀 90→138 全過；**round2＝放置 35s 後再點**（歷史凍結場景）kernel touch 捕獲+focus 重開 Settings+幀 168→206＝**S112 RefCountPipe 修法在真實使用者場景下成立**。unit 24/24。⑤ **第二個 Quick Boot 日常路徑 bug**：`adbPathForConfig` 從 emulatorPath 推 adb，`-Fast` custom runtime 推出不存在的 `build\platform-tools\adb.exe` → graceful `adb emu kill` 靜默失敗 → 按 X 關窗走 TerminateProcess＝**snapshot 永不保存、下次全是 34s 冷開**（實證：使用者關閉後 snapshot 檔案 mtime 未動）。修法＝fallback `sdkRootForConfig`/platform-tools。E2E `pass-close-saves-quickboot`（WM_CLOSE→snapshot.pb mtime 前進→重開 **8.5s** 快載 alive）。教訓：**verifier 必須跑出貨 flavor**；guest 掛死時 `adb shell` 會永久 hang，等待迴圈一律加 timeout；**evdev 對未變化 ABS 值去重**（同座標第二次 touch 不帶 POSITION_X/Y），送達證據要看 TRACKING_ID 非座標。
- **Session 112（本次）——「有畫面但點不動」真根因根治（RefCountPipe）＋確定性重現器＋啟動消音**：/goal「修好專案真正可用、超越 BlueStacks」。① **根治連續六個 session 的停更 bug**：真根因＝gfxstream DLL 因 cross-DLL FeatureSet no-op 把 `m_refCountPipeEnabled` 硬編 false，但 emulator hosts refcount pipe（advancedFeatures.ini on）且 API-34 guest gralloc 走 pipe 模式（不送 rcOpen/CloseColorBuffer）→ legacy refcount 對不參與的 guest 亂記帳：headless post 每幀讓 display CB 瞬時 refcount 0→10s delayed-close，活躍時每幀救回、**guest 靜止 ≥10s 即被 sweep 銷毀→producer 永久停更**（「放著回來就點不動」；歷來 verifier 從不靜止 10s＝15+ session 盲區）。修法＝`Impl::Create` force true（kill switch `CHIMERA_GFXSTREAM_NO_REFCOUNT_PIPE=1`、patch script 同步冪等）。驗證＝確定性重現器（boot→20s 靜止→操作）：未修 3/3 RED、fix 3/3 GREEN、**同 build kill switch 2/2 RED＝A/B 因果定案**；unit 24/24、SelfTest pass（boot 35s/nonblack 100%/interactivity ok）、**idle-recovery gate pass**（production 30s 放置×2、恢復 host total 118→266）。② 新診斷資產：`rcFBPost`→`postImpl`→`published sequence` 三層 throttled 計數＋postImpl missing-CB log（原唯一無 log 靜默失敗點）。③ 啟動「鈴聲」＝guest 充電提示音/解鎖音，`applyGuestFirstBootSetup` 關閉（charging/lockscreen/sound_effects）。④ 競品對照與「超越」量測指標入 `competitor-emulator-smoothness.md` §9。詳見 `CONTEXT.md`。
- **Session 111——「有畫面但點不動」真根因更正 + fallback 保命 + README 降調**：使用者再次回報一鍵開啟後「有畫面但完全點不動」。本輪現場證據推翻先前輸入方向：`CHIMERA_GRPC_INPUT_DIAG` 顯示 `sendTouch` 到正確 gRPC 8570 且 `http200 grpc-status0`；guest `adb shell getevent -lt` 同步捕到 `virtio_input_multi_touch_1` down/up＝**輸入有進 guest kernel**。同時 ADB tap / `am start android.settings.SETTINGS` 可改變 guest screencap，但 host `CHIMERA_PERF total` 停在 118/122，shared texture 在 `Failed to find ColorBuffer ... invalidateGl/invalidateVk` 後不再 `published sequence`＝**真根因是 `-Fast` shared-texture producer 停更，使用者看到舊畫面，誤以為點不動**。修法：`src/host/ui/main.cpp` ① shared texture 模式下仍建立 unary gRPC capture；② 新增 **shared-texture stall watchdog**（1s tick、`kSharedTextureStallMs=6000`）——producer 超時沒發幀就啟動 gRPC 保命，恢復即停回。**兩條 capture 不可同時常駐**（`setFrame`/`setSharedD3D11Texture` 會互相清狀態），故由 watchdog 二選一。新增 `test-ui-main-port-contract` 鎖住 wiring 與 watchdog。驗證：contract RED→GREEN、unit **24/24 PASS**、live `stalled for 6664 ms; resuming gRPC capture`→`producer recovered; parking`、host total **126→215**。誠實限制：閒置無幀與 producer 死亡在「沒幀」這個訊號上無法區分，門檻只是取捨（2s 閒置誤觸 5 次→6s 降為 1 次），最壞有 6s 舊畫面；這是可用性補丁非 gfxstream 根治。README 已重寫成誠實版：本專案目前不是可日常/正式使用的模擬器，只是 AI-assisted emulator experiment / 整活專案。詳見 `CONTEXT.md`。
- **Session 110——code review + 清死碼審查 + 垃圾清理 + 持續測試（無程式碼變更）**：使用者「審查代碼／精簡清死碼（不要額外產出 BUG）／更新文件／清垃圾／持續測試」。① **Code review**：逐行審近期 churn（S108 輸入、S109b `generate-chimera-vk-ext-handlers.py`）＝乾淨——產生器五路對稱（同一 `gen` POD universe）、確定性（`sorted`+`setdefault`）、idempotent（`write()` 內容同不覆寫、`strip_blocks` 先清）、size_t BE64 三路對稱；`InputBridge` console 移除後無孤兒 member。② **清死碼＝無可清（誠實）**：`src/host` 的 `-prop`/`legacy`/`Q_UNUSED` 皆刻意（說明註解／rollback 路徑／Qt 樁），`--qemu-backend`/`--hcs-backend`/proxy 屬 R&D「保留不刪」，`src/virtualization/qemu/` 是 vendored 上游 fork——強行刪會違反「不要額外產出 BUG」與保留決策。③ **垃圾清理**：`tmp/` loose 診斷 throwaway 65 檔/9.1MB + `scripts/__pycache__` 刪除（保留 active gfxstream build trees）。④ **持續測試**：unit **24/24 PASS**（7.84s，清理零破壞）；互動 UI（below_normal 出貨預設）display path 健康（gpu-direct/sharedTexture=yes/fallback=none/nonblack 100%）、sustained-scroll effFps 49.3/effMin 45.8/dup 0＝not-60 屬 guest scroll cadence run-to-run 變異非回歸；**定義性 1080p60（gl60 continuous、normal priority）乾淨隔離重跑 min 59.0/avg 59.9/dup 0 pass**（首跑 45.6 是與互動 verifier teardown 重疊的瞬時競爭＝量測教訓：perf gate 必須單獨隔離跑）。詳見 `CONTEXT.md`。
...
- **一鍵啟動**：根目錄 `start-chimera.cmd` = `start-chimera.ps1 -Fast`（custom gfxstream shared texture + `CHIMERA_GUEST_VULKAN=1`＝只加 `-feature Vulkan`；priority 預設 below_normal 護 host audio，S108 定案）；`-InteractiveFirst` 換最順（音訊代價）、`-AudioFirst` 最省音訊、`-Stock` 才回 stock gRPC 慢路徑（~4–17 FPS）、`-NoQuick` 強制全冷開。**啟動時間（S112 Quick Boot 預設化）**：首次冷開 `boot≈34s`（關閉時自動存 `default_boot`、關閉多等幾秒存檔），之後 **`boot≈7.5s`**（`verify-quick-boot` 三階段實測 full 34.3 / seed 30.9 / quick 7.5 pass）；SelfTest 固定全冷開保持數字可比。boot 期間 QML placeholder 由 `AndroidControls.bootReady` 撐到 boot 完成，launcher 以 md5 比對跳過重複 reinstall。
...
| `CHIMERA_QUICK_BOOT` / `CHIMERA_SAVE_QUICK_BOOT` | `0\|1` | 空 | **S112 起 `start-chimera.ps1` 預設設 1**（`-NoQuick`/SelfTest 除外）：QUICK_BOOT=1＝**AVD default Quick Boot**（不帶任何 snapshot 旗標；emulator 自管 `default_boot`：關機存、unclean exit 自動冷開不回捲、config 變更自動失效）＋`stop()` 走優雅 `adb emu kill`（等 ≤20s 讓存檔完成）。實測 34.3s→**7.5s**。**禁止改回 named `-snapshot`**（load 會回捲 qcow2 磁碟＝資料遺失）。SAVE_QUICK_BOOT 僅供 verifier 額外存 legacy named snapshot。**S112c**：跨 display flavor（stock↔-Fast）的 snapshot 會 brick guest→`VirtualMachine::start()` 以 `chimera_display_flavor.txt` marker 自動 invalidate（flavor 變＝刪 `default_boot` 冷開一次） |
...
| 問題 | 狀態 |
|------|------|
| Quick Boot 跨組態 snapshot 污染→開機整台掛死 | RESOLVED — S112c：stock flavor 存的 `default_boot` 被 `-Fast`（custom gfxstream DLL）載入＝guest 永久 hang（load 回報成功、adb shell 掛死、qemu 幾乎零 CPU）；emulator 相容檢查看不到 DLL/env 差異。修法＝`VirtualMachine.cpp` display-flavor marker（`chimera_display_flavor.txt`），flavor 變即刪 `default_boot` 冷開。A/B `pass-flavor-marker-ab`；`-Fast` 自洽 3 階段 pass（載入 8.5s 活）。**同輪第二 bug**：`adbPathForConfig` 對 custom runtime 推出不存在的 adb→按 X 關窗 graceful kill 靜默失敗→snapshot 永不存＝日常全冷開；修＝SDK fallback，E2E `pass-close-saves-quickboot`（WM_CLOSE→存檔→8.5s 快載）。教訓：verifier 必須跑出貨 flavor |
| 有畫面但完全無法點擊（看似無反應） | **RESOLVED (S112) — 真根因＝gfxstream `m_refCountPipeEnabled` 硬編 false 的 RefCountPipe 失衡**：emulator hosts refcount pipe（advancedFeatures.ini on）、API-34 guest gralloc 走 pipe 模式（不送 rcOpen/CloseColorBuffer），host 卻跑 legacy refcount＝post-O CB create refcount=0、headless post 每幀瞬時歸 0 進 10s delayed-close；活躍時每幀 markOpened 救回，**guest 靜止 ≥10s 後 display 鏈 CB 被 sweep 銷毀→producer 永久停更**（「放著回來就點不動」；歷來 verifier 從不靜止 10s 所以 15+ session 抓不到）。修法：`Impl::Create` force `m_refCountPipeEnabled=true`（kill switch `CHIMERA_GFXSTREAM_NO_REFCOUNT_PIPE=1`；patch script 同步）。驗證＝確定性重現器（boot→20s 靜止→操作，`svc power stayon true` 排除螢幕睡眠擬態）：fix 3/3 GREEN（rcFBPost=postImpl=kvk lockstep、seq 1200）、**同 build kill switch 2/2 RED＝A/B 因果定案**；production idle-recovery gate pass。診斷資產：`rcFBPost`/`postImpl`/`published sequence` 三層 throttled 計數＋postImpl missing-CB log（原唯一無 log 靜默失敗點）。S111 watchdog 保留（belt-and-suspenders）。以下為歷史誤判紀錄 — S108 舊定案：**兩個歷任根因皆錯**（S106 埠被搶：netstat 乾淨、POST 全 200；S107 -Fast 顯示凍結：150s 出貨組態實測 producer 1→4800、host total ~1:1 緊跟、螢幕 hash 30 distinct＝完全健康，`Failed to find ColorBuffer` 是 throttled 無害噪音）。**輸入路徑端到端證實完好**（S107：真實 WM_LBUTTONDOWN→sendTouch→guest 開 SearchActivity；gRPC 3138 POST 全 200）。受控測試無法重現原始症狀。**S108 defense-in-depth 完成＝不論成因結構性自癒**：gRPC 3-strike breaker（2s POST timeout 抓 hang 型死亡）→ ADB tap fallback → `getStatus` 探測自動復原；劫持 E2E `fallback-pass` 實證（點擊在通道死亡下仍送達 guest）。**若再發生**：設 `CHIMERA_GRPC_INPUT_DIAG` + 三指標 liveness（producer `kVk GPU path` frameN vs `CHIMERA_PERF total` vs 螢幕 BitBlt hash 隨時間對照）當場拆帳，不可只看單張截圖 |
...
| `scripts/verify-true-1080p60.ps1` | 連續渲染 runtime gate（gl60 synthetic，非日常 UI）；`-HeavyIterations N` 量 GLES/SwiftShader fill 天花板。**S109c 更正：嚴格 60 gate 過不過的唯一槓桿是 priority——預設 below_normal 下 min≈52 過不了 57/59 gate，`CHIMERA_INTERACTIVE_PRIORITY=normal` 下 min 59.1/avg 60.0/dup 0 乾淨過**（非架構性不可過）。任何數字仍要配 host 視窗像素證據 |
...
---
*Updated: 2026-07-10 — Session 112c：Quick Boot 跨組態 snapshot 污染 brick 根治（display-flavor marker 進 VirtualMachine::start()，A/B pass-flavor-marker-ab）＋人類保真度點擊 gate（真實 WM_LBUTTONDOWN、35s 放置後仍可點）。S112 主體：「有畫面但點不動」真根因根治＝gfxstream RefCountPipe 失衡（m_refCountPipeEnabled 硬編 false vs pipe 模式 guest；idle ≥10s 後 display CB 被 delayed-close sweep 銷毀→producer 永久停更）。修法 force true（kill switch CHIMERA_GFXSTREAM_NO_REFCOUNT_PIPE=1、patch script 同步）；同 build A/B 因果定案（fix 3/3 GREEN、killswitch 2/2 RED）；unit 24/24、SelfTest pass、idle-recovery gate pass。新診斷：rcFBPost→postImpl→published sequence 三層計數。啟動鈴聲（guest 充電/解鎖音）已關。歷史 session 摘要見 CONTEXT.md。*
