...
| 問題 | 狀態 |
|------|------|
| 有畫面但完全無法點擊（觸控全丟） | RESOLVED — Session 106：根因＝gRPC 輸入埠 8554 被 orphan/舊 listener 搶。guest 觸控正常（headless 實證）；顯示走 shared texture 獨立於 gRPC，故「畫面在、點擊全丟」。`m_grpcInput` 恆非 null＝ADB fallback 死碼＋固定 5554→8554（正常路徑不挑空埠）＝POST 打到搶埠者全丟。修法：`start-chimera.ps1` 正常啟動自動挑空埠、`Get-FreeEmulatorConsolePort` 加驗衍生 gRPC 埠（免重編）；占用 8554 端到端驗證自挑 5580→8580、synthetic scroll `pass-gpu-direct-60`。app 端 gRPC 健康檢查/ADB 回退＝未做的 defense-in-depth（直開 exe 才需，使用者不走） |
